...
export default function Chatbot() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:8000/chatbot/", { message: input }, {
        headers: { Authorization: `Bearer ${token}` },
      });
      
      // Format bot response and add to chat
      const botMessage = { sender: "bot", text: res.data.reply.replace(/\n/g, "<br/>").replace(/\*\*(.*?)\*\*/g, "<b>$1</b>") };
      setMessages((prevMessages) => [...prevMessages, botMessage]);
    } catch (err) {
      // Handle API error by displaying an error message in the chat
